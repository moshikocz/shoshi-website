...
(function () {
  ...
  for (var ci = 0; ci < cloneCount; ci++) {
    ...
    // An aria-hidden element must not contain focusable descendants — the
    // cloned slide's "קראי עוד" button would otherwise still catch Tab focus.
    clone.querySelectorAll('button, a, input, textarea, select, [tabindex]').forEach(function(node) {
