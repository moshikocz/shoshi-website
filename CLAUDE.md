...
- Shoshi edits some content herself through `/admin` (Decap CMS, config in `admin/config.yml`), authenticated via GitHub OAuth (`netlify/functions/auth.js` + `callback.js`) — never Netlify Identity, and she never needs to see GitHub directly. Her saves are real git commits to `main`; **check `git status`/`git log` at the start of a session** to catch her edits before making local changes (see the CMS memory notes for this project).
